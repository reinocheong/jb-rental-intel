...
> 从 FB 抓取 agent 号码 → WhatsApp 自动推广 → 试用 → Stripe 付费续费，全链路自动化。
...
```
FB 群组帖子 ──→ 提取 agent 电话 ──→ WhatsApp 推广 ──→ 3天免费试用 ──→ RM 9.90/月自动续费
     ↑ scraper/          ↑ processors/       ↑ outreach/      ↑ sub_mgr.py    ↑ stripe_checker.py
```

> ✅ 五阶段全部完成，全自动流水线已运行中。每日排程见下方「每日自动运行表」。
...
│
├── 🔧 processors/                   ← 阶段②：数据解析
│   ├── fb_parser.py                 # ★ 主路径 → Google Sheets（647行）
...
| 2026-05-14 | 推广文案A回复率0%（24条发送0回复），定位偏差（强调"工具功能"而非cobroke价值） | 重写文案A/B为cobroke定位，outreach_engine改为A/B交替发送，推广记录标记模板列用于对比 |
| 2026-05-14 | 客户看 Sheet 体验差（表格在手机难以浏览） | 新建 `rentals.html` 手机端房源浏览页 + `scripts/export_rentals_json.py` 自动导出 JSON，每30分钟更新。只读 Sheet，不写不改。
