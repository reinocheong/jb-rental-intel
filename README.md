...
|| 2026-05-16 | 14个纯脚本cron全切no_agent零模型运行，月API费从~¥75降至~¥0.30 | 删form检查（已不用），13个cron转为纯脚本模式 |
|| 2026-05-18 | 爬虫 Group5-租屋 `browser has been closed` 报错 + 每个群组独立启动5个Chromium超时 | 重构 fb_scraper.js：① 全局复用1个浏览器 ② finally 块逐层 try/catch 防级联崩溃 ③ 每群组60s超时保护 |
|| 2026-05-18 | 爬虫+解析器串行跑超过 cron 120s 限制，定时任务一直 error | 拆分 cron：爬虫 `:00/:30` → 解析器 `:03/:33`（3分钟后）。爬虫加 FB 浏览器死掉自动 relaunch + retry |